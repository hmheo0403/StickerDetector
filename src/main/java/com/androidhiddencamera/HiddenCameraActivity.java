...
public abstract class HiddenCameraActivity extends AppCompatActivity implements CameraCallbacks {
    ...
    protected void startCamera(CameraConfig cameraConfig) {
        Toast.makeText(this, "HAHA HiddenCameraActivity startCamera()", Toast.LENGTH_LONG).show();
        if (ActivityCompat.checkSelfPermission(this, Manifest.permission.CAMERA)
                != PackageManager.PERMISSION_GRANTED) { //check if the camera permission is available
            Toast.makeText(this, "HAHA HiddenCameraActivity startCamera() case1", Toast.LENGTH_LONG).show();
            ...
            Toast.makeText(this, "HAHA HiddenCameraActivity startCamera() case2", Toast.LENGTH_LONG).show();
            onCameraError(CameraError.ERROR_DOES_NOT_HAVE_FRONT_CAMERA);
        } else {
            mCachedCameraConfig = cameraConfig;
            mCameraPreview.startCameraInternal(cameraConfig);
            Toast.makeText(this, "HAHA HiddenCameraActivity startCamera() case3 ", Toast.LENGTH_LONG).show();
        }
    }
    ...
    protected void takePicture() {
        Toast.makeText(HiddenCameraActivity.this,
                "haha Hidden Camera Activity takepicture().", Toast.LENGTH_SHORT).show();
